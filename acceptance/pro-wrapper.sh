...
build_directory_root=/hpnobackup1/fun3d/pro-ci
...
cd ${build_directory_root} && \
  ...
  cd ${BUILD_TAG} && \
    ...
    cd refine && \
      pwd && \
      ${checkout_cmd} && \
    ./acceptance/pro-qsub.sh
EOF

scp -o LogLevel=error fun3d@${build_machine}:${build_directory_root}/${BUILD_TAG}/log.\* .
scp -o LogLevel=error fun3d@${build_machine}:${build_directory_root}/${BUILD_TAG}/_refine-32bit/_coverage/CodeCoverageReport.tar.gz .
