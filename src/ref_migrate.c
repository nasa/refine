...
REF_STATUS ref_migrate_create(REF_MIGRATE *ref_migrate_ptr, REF_GRID ref_grid) {
  ...
  each_ref_grid_ref_cell(ref_grid, group, ref_cell)
      each_ref_cell_valid_cell(ref_cell, cell) each_ref_cell_cell_edge(
          ref_cell, cell_edge) { /* need ghost nodes for agglomeration */
...
REF_STATUS ref_migrate_new_part(REF_GRID ref_grid) {
#ifdef HAVE_ZOLTAN
  {
    ...
    REIS(ZOLTAN_OK,
         Zoltan_LB_Partition(zz, &partitions_have_changed, &global_id_dimension,
                             ...
                             &import_local, &import_proc, &import_part,
                             &export_n, &export_global, &export_local,
                             &export_proc, &export_part),
         "Zoltan is angry");

    ref_malloc_init(migrate_part, ref_migrate_max(ref_node), REF_INT,
                    REF_EMPTY);
...

  {
    ...
    ref_malloc_init(tpwgts, ref_mpi_m(ref_mpi), PARM_REAL,
                    1.0 / (REF_DBL)ref_mpi_m(ref_mpi));
    ref_malloc_init(ubvec, ref_mpi_m(ref_mpi), PARM_REAL, 1.01);
