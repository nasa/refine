...
class TestGrid < Test::Unit::TestCase
 ...
 def test_BarycentricCoordinate_of_vertices
  ...
  bary = @grid.barycentricCoordinate(xyz0,xyz1,xyz2,xyz3,xyz0)
  ans = [1.0,0.0,0.0,0.0]
  4.times { |i| assert_in_delta( ans[i], bary[i], tol, 'element '+i.to_s ) }

  bary = @grid.barycentricCoordinate(xyz0,xyz1,xyz2,xyz3,xyz1)
  ans = [0.0,1.0,0.0,0.0]
  4.times { |i| assert_in_delta( ans[i], bary[i], tol, 'element '+i.to_s ) }

  bary = @grid.barycentricCoordinate(xyz0,xyz1,xyz2,xyz3,xyz2)
  ans = [0.0,0.0,1.0,0.0]
  4.times { |i| assert_in_delta( ans[i], bary[i], tol, 'element '+i.to_s ) }

  bary = @grid.barycentricCoordinate(xyz0,xyz1,xyz2,xyz3,xyz3)
  ans = [0.0,0.0,0.0,1.0]
  4.times { |i| assert_in_delta( ans[i], bary[i], tol, 'element '+i.to_s ) }
 end
