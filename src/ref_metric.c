...
REF_STATUS ref_metric_belme_gk(REF_DBL *metric, REF_GRID ref_grid, REF_INT ldim,
                               REF_DBL *prim_dual_dfdq,
                               REF_RECON_RECONSTRUCTION reconstruction) {
  REF_NODE ref_node = ref_grid_node(ref_grid);
  REF_INT var, node, i, nequ;
  ...
  var = 0;
  each_ref_node_valid_node(ref_node, node) {
    cons[node] = prim_dual_dfdq[0 + ldim * node];
  }
  RSS(ref_recon_hessian(ref_grid, cons, hess_cons, reconstruction), "hess_u");
  each_ref_node_valid_node(ref_node, node) {
    for (i = 0; i < 6; i++) {
      metric[i + 6 * node] +=
          ABS(prim_dual_dfdq[var + 2 * nequ + ldim * node]) *
          hess_cons[i + 6 * node];
    }
  }
  ...
  for (var = 1; var <= 3; var++) {
    each_ref_node_valid_node(ref_node, node) {
      cons[node] =
          prim_dual_dfdq[0 + ldim * node] * prim_dual_dfdq[var + ldim * node];
    }
    RSS(ref_recon_hessian(ref_grid, cons, hess_cons, reconstruction), "hess_u");
    each_ref_node_valid_node(ref_node, node) {
      for (i = 0; i < 6; i++) {
