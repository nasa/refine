...
REF_STATUS ref_metric_kexact_hessian(REF_GRID ref_grid, REF_DBL *scalar,
  ...
  REF_DBL *a, *q, *r;
  REF_INT m, n;
  REF_BOOL normal_equation = REF_FALSE;
  each_ref_node_valid_node(ref_node, node0) {
    ...
    if (normal_equation) { /* use normal equations */
      for (i = 0; i < 90; i++) ab[i] = 0.0;
      each_ref_dict_key(ref_dict, i2, node2) {
        dx =
            ref_node_xyz(ref_node, 0, node2) - ref_node_xyz(ref_node, 0, node0);
        dy =
            ref_node_xyz(ref_node, 1, node2) - ref_node_xyz(ref_node, 1, node0);
        dz =
            ref_node_xyz(ref_node, 2, node2) - ref_node_xyz(ref_node, 2, node0);
