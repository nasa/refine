...
REF_STATUS ref_node_tattle_global( REF_NODE ref_node, REF_INT global )
{
  ...
  printf("%d: global %d: search%d %d exhast%d %d\n",
	 ref_mpi_rank(ref_node_mpi(ref_node)), global,
...
REF_STATUS ref_node_eliminate_unused_globals( REF_NODE ref_node )
{
  ...
  RSS( ref_list_allgather( ref_list, 
			   ref_node_mpi(ref_node) ), "gather unused global" );
...
REF_STATUS ref_node_ghost_real( REF_NODE ref_node )
{
  RSS( ref_node_ghost_dbl( ref_node,
			   ref_node->real, REF_NODE_REAL_PER ), "ghost dbl" );
  return REF_SUCCESS;  
}
...
REF_STATUS ref_node_tri_jac_dquality_dnode0( REF_NODE ref_node, 
					 REF_INT *nodes, 
					 REF_DBL *quality, 
					 REF_DBL *d_quality )
{
					 ...
					 REF_DBL *d_quality )
{
  switch (ref_node->tri_quality)
    ...
    case REF_NODE_EPIC_QUALITY:
      ...
      break;
    case REF_NODE_JAC_QUALITY:
      RSS( ref_node_tri_jac_dquality_dnode0(ref_node,nodes,
					    quality,d_quality), "epic");
      break;
    default:
      THROW("case not recognized");
  ...
  if ( ref_math_divisible(bary[0],total) &&
	...
	bary[i] = 0.0;
      ...
      printf("%s: %d: %s: div zero tot %.18e %.18e vol %.18e %.18e %.18e %.18e\n",
	     __FILE__,__LINE__,__func__,
	     total, volume, bary[0], bary[1], bary[2], bary[3] );
      return REF_DIV_ZERO;
    }
