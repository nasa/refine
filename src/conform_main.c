...
#define PRINT_STATUS {double l0,l1;gridEdgeRatioRange(grid,&l0,&l1);printf("Len %9.2e %9.2e AR %12.5e Vol %10.6e\n", l0,l1, gridMinThawedAR(grid), gridMinVolume(grid)); fflush(stdout);}
...
#define STATUS { \
  PRINT_STATUS; \
  printf("%10d %12.5e %%oct\n",gridNNode(grid),sqrt(gridMinThawedAR(grid))); \
}
