...
REF_STATUS ref_list_apply_offset(REF_LIST ref_list, REF_INT equal_and_above,
  ...
  for (i = 0; i < ref_list_n(ref_list); i++)
    if (ref_list->value[i] >= equal_and_above) ref_list->value[i] += offset;
...
REF_STATUS ref_list_sort(REF_LIST ref_list) {
  ...
  RSS(ref_sort_heap_int(ref_list_n(ref_list), ref_list->value, order), "heap");

  for (i = 0; i < ref_list_n(ref_list); i++)
    order[i] = ref_list->value[order[i]];

  for (i = 0; i < ref_list_n(ref_list); i++) ref_list->value[i] = order[i];
...
REF_STATUS ref_list_allgather(REF_LIST ref_list, REF_MPI ref_mpi) {
  ...
  for (i = 0; i < ref_list_n(ref_list); i++) local_copy[i] = ref_list->value[i];
  ...
  RSS(ref_mpi_allgatherv(ref_mpi, local_copy, counts, ref_list->value,
                         REF_INT_TYPE),
      "gather values");
