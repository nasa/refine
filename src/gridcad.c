...
Grid *gridLineSearchTForCost(Grid *grid, int node )
{
  double dt, t;
  double gold;
  double alpha[2], ar[2];
...
Grid *gridSmoothNodeARFace(Grid *grid, int node )
{
  if (!gridGeometryFace(grid,node)) return NULL;
  if (gridGeometryBetweenFace(grid,node)) return grid;
...
static double reflectNodeARFace( Grid *grid,
...
Grid *gridSmoothNodeARFaceSimplex( Grid *grid, int node )
{
  int evaluations;
  int s, i;
