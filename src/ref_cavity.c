...
REF_STATUS ref_cavity_create(REF_CAVITY *ref_cavity_ptr, REF_INT node_per) {
  ...
  ref_malloc_init(ref_cavity->f2n,
                  ref_cavity_max(ref_cavity) * ref_cavity_node_per(ref_cavity),
                  REF_INT, 0);
...
REF_STATUS ref_cavity_inspect(REF_CAVITY ref_cavity) {
  ...
  printf("node_per = %d n = %d max = %d blank = %d\n",
         ref_cavity_node_per(ref_cavity), ref_cavity_n(ref_cavity),
...
REF_STATUS ref_cavity_insert(REF_CAVITY ref_cavity, REF_INT *nodes) {
  ...
  if (REF_EMPTY == ref_cavity_blank(ref_cavity)) {
    ...
    ref_realloc(ref_cavity->f2n,
                ref_cavity_node_per(ref_cavity) * ref_cavity_max(ref_cavity),
                REF_INT);
...
REF_STATUS ref_cavity_add_tri(REF_CAVITY ref_cavity, REF_GRID ref_grid,
  ...
  RSS(ref_list_push(ref_cavity_list(ref_cavity), tri), "save tri");

  RSS(ref_cell_nodes(ref_grid_tri(ref_grid), tri, nodes), "grab faceid");
...
REF_STATUS ref_cavity_rm_tri(REF_CAVITY ref_cavity, REF_GRID ref_grid,
  ...
  RSS(ref_list_delete(ref_cavity_list(ref_cavity), tri), "dump tri");
...
REF_STATUS ref_cavity_replace_tri(REF_CAVITY ref_cavity, REF_GRID ref_grid,
  ...
  if (0 == ref_list_n(ref_cavity_list(ref_cavity))) return REF_INVALID;

  cell = ref_list_value(ref_cavity_list(ref_cavity), 0);
  RSS(ref_cell_nodes(ref_grid_tri(ref_grid), cell, nodes), "grab faceid");
  faceid0 = nodes[3];
  RSS(ref_twod_tri_pri_tri(ref_grid_tri(ref_grid), ref_grid_pri(ref_grid), cell,
                           &pri, &tri),
      "tpt");
  RSS(ref_cell_nodes(ref_grid_tri(ref_grid), tri, nodes), "grab faceid");
  faceid1 = nodes[3];
  ...
  each_ref_cavity_valid_face(ref_cavity, face) {
    ...
    RSS(ref_twod_opposite_edge(
            ref_grid_pri(ref_grid), ref_cavity_f2n(ref_cavity, 0, face),
            ref_cavity_f2n(ref_cavity, 1, face), &node2, &node3),
        "opp");
    nodes[0] = node3;
    nodes[1] = node2;
    nodes[2] = clone;
  ...
  while (ref_list_n(ref_cavity_list(ref_cavity)) > 0) {
    ...
    RSS(ref_cell_nodes(ref_grid_tri(ref_grid), cell, nodes), "save nodes");
    RSS(ref_cell_remove(ref_grid_tri(ref_grid), cell), "rm");
    each_ref_cell_cell_node(
        ref_grid_tri(ref_grid),
        cell_node) if (ref_cell_node_empty(ref_grid_tri(ref_grid),
                                           nodes[cell_node])) {
      RSS(ref_node_remove(ref_grid_node(ref_grid), nodes[cell_node]), "rm");
      RSS(ref_geom_remove_all(ref_grid_geom(ref_grid), nodes[cell_node]), "rm");
    }

    RSS(ref_cell_nodes(ref_grid_tri(ref_grid), tri, nodes), "save nodes");
  ...
  switch (ref_cavity_node_per(ref_cavity)) {
    ...
    case (3):
      ...
      for (cell = 0; cell < ncell; cell++) {
        RSS(ref_cavity_add_tet(ref_cavity, ref_grid, cell_to_add[cell]),
            "insert");
      }
      break;
    default:
  ...
  switch (ref_cavity_node_per(ref_cavity)) {
    case (2):
      RSS(ref_cell_list_with2(
              ref_grid_tri(ref_grid), ref_cavity_f2n(ref_cavity, 0, face),
              ref_cavity_f2n(ref_cavity, 1, face), 2, &ncell, cells),
          "more than two");
      if (0 == ncell) THROW("cavity triangle missing");
      if (1 == ncell) return REF_INVALID; /* boundary */
      RSS(ref_list_contains(ref_cavity_list(ref_cavity), cells[0], &have_cell0),
          "cell0");
      RSS(ref_list_contains(ref_cavity_list(ref_cavity), cells[1], &have_cell1),
          "cell1");
      if (have_cell0 == have_cell1) THROW("cavity same state");
      if (have_cell0)
        RSS(ref_cavity_add_tri(ref_cavity, ref_grid, cells[1]), "add c1");
      if (have_cell1)
        RSS(ref_cavity_add_tri(ref_cavity, ref_grid, cells[0]), "add c0");
      break;
    case (3):
      ...
      if (have_cell0 == have_cell1) THROW("cavity same state");
      if (have_cell0)
        RSS(ref_cavity_add_tet(ref_cavity, ref_grid, tet1), "add c1");
      if (have_cell1)
        RSS(ref_cavity_add_tet(ref_cavity, ref_grid, tet0), "add c0");
      break;
    default:
...
REF_STATUS ref_cavity_shrink_face(REF_CAVITY ref_cavity, REF_GRID ref_grid,
  ...
  switch (ref_cavity_node_per(ref_cavity)) {
    case (2):
      ...
      if (have_cell0 == have_cell1) THROW("cavity same state");
      if (!have_cell0)
        RSS(ref_cavity_rm_tri(ref_cavity, ref_grid, cells[1]), "rm c1");
      if (!have_cell1)
        RSS(ref_cavity_rm_tri(ref_cavity, ref_grid, cells[0]), "rm c0");
      break;
    case (3):
      face_nodes[0] = ref_cavity_f2n(ref_cavity, 0, face);
      face_nodes[1] = ref_cavity_f2n(ref_cavity, 1, face);
      ...
      if (have_cell0 == have_cell1) THROW("cavity same state");
      if (!have_cell0)
        RSS(ref_cavity_rm_tet(ref_cavity, ref_grid, tet1), "add c1");
      if (!have_cell1)
        RSS(ref_cavity_rm_tet(ref_cavity, ref_grid, tet0), "add c0");
      break;
    default:
...
REF_STATUS ref_cavity_local(REF_CAVITY ref_cavity, REF_GRID ref_grid,
  ...
  REF_INT face, face_node;
  REF_CELL ref_cell = ref_grid_tet(ref_grid);
  if (2 == ref_cavity_node_per(ref_cavity)) ref_cell = ref_grid_tri(ref_grid);
