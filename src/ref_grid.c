...
static REF_STATUS ref_update_tri_guess(REF_CELL ref_cell, REF_INT node0,
  ...
  RSS(ref_cell_list_with2(ref_cell, node0, node1, max_cell, &ncell, cell_list),
      "next");
  if (REF_EMPTY == cell_list[0]) THROW("bary update missing first");
  if (REF_EMPTY == cell_list[1]) { /* hit boundary */
    *guess = REF_EMPTY;
    return REF_SUCCESS;
  }
...
REF_STATUS ref_grid_enclosing_tri(REF_GRID ref_grid, REF_DBL *xyz, REF_INT *tri,
  ...
  for (step = 0; step < limit; step++) {
    if (!ref_cell_valid(ref_cell, guess)) {
      RSS(ref_grid_exhaustive_enclosing_tri(ref_grid, xyz, tri, bary),
          "enclose");
      return REF_SUCCESS;
    }
  ...
  RSS(ref_grid_exhaustive_enclosing_tri(ref_grid, xyz, tri, bary), "enclose");
